...
import cors from "cors";
import { WebSocketServer, WebSocket } from "ws"; // Add this import
...
wss.on("connection", ws => {
	...
	// Send initial streams data
	ws.send(
		JSON.stringify({
			type: "streams-update",
			streams: streamHandler.getActiveStreams()
		})
	);
...
function broadcastStreamsUpdate() {
	const streams = streamHandler.getActiveStreams();
...
const init = async () => {
	...
	// Add broadcast on stream updates
	streamHandler.onUpdate = broadcastStreamsUpdate;
	...
	httpServer.listen(httpPort, "0.0.0.0", () => {
		console.log(`\nServer Info:`);
		console.log(`- HTTP API: http://${serverAddress}:${httpPort}`);
		console.log(`- WebSocket: ws://${serverAddress}:${httpPort}/ws`);
		console.log(`- RTSP Port: ${rtspPort}`);
		console.log(
			`- Stream format: rtsp://${serverAddress}:${rtspPort}/live/<streamName>`
		);
